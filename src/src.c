 ...
 */
static Fnt *drw_font_xcreate(Drw *drw, const char *fontname,
...
inline void drw_clr_free(Clr *clr) { free(clr); }

inline void drw_setscheme(Drw *drw, ClrScheme *scheme) { drw->scheme = scheme; }
...
int drw_get_width(Drw *drw, int numcolors, const char *text) {
  ...
  for (unsigned int i = 0; i < strlen(text); i++) {
    if (text[i] > 0 && text[i] <= numcolors) {
       ...
       * drw_text counted it as a normal character and added one character's
       * width we aren't going to render this character, so we remove one
       * character's width */
      w -= curfont->xfont->max_advance_width;

      if (i == 0 || i + 1 == strlen(text)) {
        /* we're on the first or the last character of the string
         * drw_text already added one character's height (divided by 2) as
         * padding to the beginning and end we don't want to double this
         * padding, so we skip this character */
        continue;
      }
       ...
       * we want to add one character's height (divided by 2) as padding to the
       * end of the previous colored text and to the beginning of the new
       * colored text */
      w += curfont->h;
    }
  }
...
void drw_colored_text(Drw *drw, ClrScheme *scheme, int numcolors, int x, int y,
...
void drw_rect(Drw *drw, int x, int y, unsigned int w, unsigned int h,
              int filled, int empty, int invert) {
  if (!drw->scheme)
    return;
...
int drw_text(Drw *drw, int x, int y, unsigned int w, unsigned int h,
  ...
  while (1) {
    ...
    while (*text) {
      ...
      for (i = 0; i < drw->fontcount; i++) {
        charexists = charexists || XftCharExists(drw->dpy, drw->fonts[i]->xfont,
                                                 utf8codepoint);
        if (charexists) {
    ...
    if (utf8strlen) {
      ...
      for (len = MIN((unsigned long)utf8strlen, (sizeof buf) - 1);
           len && (tex.w > w - drw->fonts[0]->h || w < drw->fonts[0]->h); len--)
        drw_font_getexts(curfont, utf8str, len, &tex);

      if (len) {
        memcpy(buf, utf8str, len);
        buf[len] = '\0';
        if (len < ((unsigned long)utf8strlen))
        ...
        if (render) {
          ...
          tx = x + (h / 2);
          XftDrawStringUtf8(
              d, invert ? &drw->scheme->bg->rgb : &drw->scheme->fg->rgb,
...
unsigned int drw_font_getexts_width(Fnt *font, const char *text,
                                    unsigned int len) {
  Extnts tex;
