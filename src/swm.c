...
void buttonpress(XEvent *e)
{
	...
	for (i = 0; i < LENGTH(buttons); i++)
		if (click == buttons[i].click && buttons[i].func &&
			...
			buttons[i].func(
				((click == ClkTagBar || click == ClkTabBar) &&
				 buttons[i].arg.i == 0) ?
					&arg :
					&buttons[i].arg);
		}
...
Monitor *createmon(void)
{
	...
	for (i = 0; i <= LENGTH(tags); i++) {
		...
		/* init layouts */
		m->pertag->ltidxs[i][0] =
			&layouts[def_layouts[i % LENGTH(def_layouts)] %
				 LENGTH(layouts)];
...
void drawbar(Monitor *m)
{
	...
	if ((w = x - xx) > bh) {
		...
		if (m->sel) {
			...
			drw_rect(drw, x + 1, 1, dx, dx, m->sel->isfixed,
				 m->sel->isfloating, 0);
		} else {
			drw_setscheme(drw, &scheme[0]);
...
void drawtab(Monitor *m)
{
	...
	if (tot_width >
	    m->ww) { // not enough space to display the labels, they need
		// to be truncated
		memcpy(sorted_label_widths, m->tab_widths,
		       sizeof(int) * m->ntabs);
	...
	do {
		...
		case MotionNotify:
			...
			if (nx >= selmon->wx && nx <= selmon->wx + selmon->ww &&
				...
				if ((const unsigned int)abs(selmon->wx - nx) <
				    snap)
					nx = selmon->wx;
				else if ((const unsigned int)abs(
						 (selmon->wx + selmon->ww) -
						 (nx + WIDTH(c))) < snap)
					nx = selmon->wx + selmon->ww - WIDTH(c);
				if ((const unsigned int)abs(selmon->wy - ny) <
				    snap)
					ny = selmon->wy;
				else if ((const unsigned int)abs(
						 (selmon->wy + selmon->wh) -
...
inline void quit(const Arg *arg)
{
	running = 0;
}
	...
	for (i = 0; i < 8; i++)
		for (j = 0; j < (unsigned int)modmap->max_keypermod; j++)
			if (modmap->modifiermap
				    [i * (unsigned int)modmap->max_keypermod +
...
void updatesystrayiconstate(Client *i, XPropertyEvent *ev)
{
	...
	sendevent(i->win, xatom[Xembed], StructureNotifyMask, CurrentTime, code,
		  0, (long)systray->win, XEMBED_EMBEDDED_VERSION);
}
...
void updatesystray(void)
{
	...
	if (!systray) {
		...
		if (!(systray = (Systray *)calloc(1, sizeof(Systray))))
			die("fatal: could not malloc() %u bytes\n",
			    sizeof(Systray));
		systray->win = XCreateSimpleWindow(dpy, root, (int)x, m->by, w,
						   (unsigned int)bh, 0, 0,
		...
		XChangeProperty(dpy, systray->win,
				netatom[NetSystemTrayOrientation], XA_CARDINAL,
				32, PropModeReplace,
		...
		if (XGetSelectionOwner(dpy, netatom[NetSystemTray]) ==
			...
			sendevent(root, xatom[Manager], StructureNotifyMask,
				  CurrentTime, (long)netatom[NetSystemTray],
				  (long)systray->win, 0, 0);
			XSync(dpy, False);
		} else {
	...
	x -= w;
	XMoveResizeWindow(dpy, systray->win, (int)x, m->by, w,
			  (unsigned int)bh);
	wc.x = (int)x;
	wc.y = m->by;
	wc.width = (int)w;
