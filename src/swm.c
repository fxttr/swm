...
Monitor *createmon(void) {
  ...
  for (i = 0; i <= LENGTH(tags); i++) {
    ...
    /* init layouts */
    m->pertag->ltidxs[i][0] =
        &layouts[def_layouts[i % LENGTH(def_layouts)] % LENGTH(layouts)];
...
void drawbar(Monitor *m) {
  ...
  for (i = 0; i < LENGTH(tags); i++) {
    ...
    drw_setscheme(
        drw,
        &scheme[(m->tagset[m->seltags] & 1 << i) ? 1 : (urg & 1 << i ? 2 : 0)]);
...
void drawtab(Monitor *m) {
  ...
  if (tot_width > m->ww) { // not enough space to display the labels, they need
    ...
    tot_width = view_info_w;
    for (i = 0; i < m->ntabs; ++i) {
      if (tot_width + (m->ntabs - i) * sorted_label_widths[i] >
          (unsigned int)m->ww)
        break;
  ...
  for (c = m->clients; c; c = c->next) {
    ...
    w = m->tab_widths[i];
    drw_setscheme(drw,
                  (c == m->sel) ? &scheme[SchemeSel] : &scheme[SchemeNorm]);
    ...
    case MotionNotify:
      ...
      if (nx >= selmon->wx && nx <= selmon->wx + selmon->ww &&
        ...
        if ((const unsigned int)abs(selmon->wx - nx) < snap)
          nx = selmon->wx;
        else if ((const unsigned int)abs((selmon->wx + selmon->ww) -
                                         (nx + WIDTH(c))) < snap)
          nx = selmon->wx + selmon->ww - WIDTH(c);
        if ((const unsigned int)abs(selmon->wy - ny) < snap)
          ny = selmon->wy;
        else if ((const unsigned int)abs((selmon->wy + selmon->wh) -
                                         (ny + HEIGHT(c))) < snap)
          ny = selmon->wy + selmon->wh - HEIGHT(c);
        if (!c->isfloating && selmon->lt[selmon->sellt]->arrange &&
            ((const unsigned int)abs(nx - c->x) > snap ||
             (const unsigned int)abs(ny - c->y) > snap))
          togglefloating(NULL);
      }
...
inline void quit(const Arg *arg) { running = 0; }
...
void updatebars(void) {
  ...
  for (m = mons; m; m = m->next) {
    ...
    m->barwin = XCreateWindow(
        dpy, root, m->wx, m->by, w, (unsigned int)bh, 0,
...
void updatestatus(void) {
  if (!gettextprop(root, XA_WM_NAME, stext, sizeof(stext)))
    strcpy(stext, "Consinder starting symo");
...
void updatesystray(void) {
  ...
  if (!systray) {
    ...
    if (!(systray = (Systray *)calloc(1, sizeof(Systray))))
      die("fatal: could not malloc() %u bytes\n", sizeof(Systray));
    systray->win =
        XCreateSimpleWindow(dpy, root, (int)x, m->by, w, (unsigned int)bh, 0, 0,
                            scheme[SchemeSel].bg->pix);
    ...
    if (XGetSelectionOwner(dpy, netatom[NetSystemTray]) == systray->win) {
      sendevent(root, xatom[Manager], StructureNotifyMask, CurrentTime,
                (long)netatom[NetSystemTray], (long)systray->win, 0, 0);
      XSync(dpy, False);
    } else {
  ...
  for (w = 0, i = systray->icons; i; i = i->next) {
    ...
    w += systrayspacing;
    i->x = (int)w;
    XMoveResizeWindow(dpy, i->win, i->x, 0, (unsigned int)i->w,
                      (unsigned int)i->h);
    w += (unsigned int)i->w;
    if (i->mon != m)
      i->mon = m;
